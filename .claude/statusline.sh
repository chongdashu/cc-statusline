#!/bin/bash
# Generated by cc-statusline v1.2.7 (https://www.npmjs.com/package/@chongdashu/cc-statusline)
# Custom Claude Code statusline - Created: 2025-08-28T12:16:22.296Z
# Theme: detailed | Colors: true | Features: directory, git, model, context, usage, session, tokens, burnrate
STATUSLINE_VERSION="1.2.7"
...
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  model_version=$(extract_json_string "$input" "version" "")
...
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  cost_usd=$(extract_json_string "$input" "total_cost_usd" "")
  total_duration_ms=$(extract_json_string "$input" "total_duration_ms" "")
